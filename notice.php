...
<head>
    ...
    <title>News & Events | Rajshahi Steam Wash</title>
...
<body>
    ...
    <div class="news text-center pt-50 pb-50">
        <div class="container">
            <table id="table" style="border: 1px solid #000">
                <thead>
                    <tr>
                        <th width=20%>Date</th>
                ...
                <tbody>
                    ...
                    // Query
                    $sql = "SELECT SQL_CALC_FOUND_ROWS * from news order by Date DESC LIMIT {$start}, {$perPage}";
                    $news = $dbh->prepare($sql);
                    $news->execute();
                    $results = $news->fetchAll(PDO::FETCH_OBJ);
                    $cnt = 1;
                    ...
                    if ($news->rowCount() > 0) {
                        foreach ($results as $result) {                ?>
                            <tr>
                                <td width=20%><?php echo htmlentities($result->date); ?></td>
                                <td width=30%><?php echo htmlentities($result->Title); ?></td>
